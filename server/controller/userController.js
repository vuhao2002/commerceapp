...
const forgotPasswordToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await user.save();
    const resetURL = `
    Hi, Please follow this link to reset Your Password. This link is valid till 10 minutes from now. <a href="http://localhost:3000/reset-password/${token}">Click Here</a>`;
    const data = {
      to: email,
...
const createOrder = asyncHandler(async (req, res) => {
  const { COD, couponApplied, address, mobile } = req.body;
  const { _id } = req.user;
  validateMongoDbId(_id);
  ...
  try {
    ...
    const findOrderUpdate = await Order.findByIdAndUpdate(
      id,
      {
        paymentIntent: {
          ...findOrder.paymentIntent,
          isPaid: status === "Delivered" ? true : false,
          status: status,
        },
        orderStatus: status,
      },
...
const getStatusOrderDelivered = asyncHandler(async (req, res) => {
  try {
...
// Total number of orders delivered
const getSumAmount = asyncHandler(async (req, res) => {
  try {
    const countOrders = await Order.find({
      orderStatus: "Delivered",
    });
    const sumAmount = countOrders.reduce((currentValue, orderItem) => {
      return orderItem.paymentIntent.amount + currentValue;
    }, 0);
    res.json(sumAmount);
  } catch (error) {
    throw new Error(error);
